...
static refalrts::FnResult func_CompileFile(refalrts::Iter arg_begin, refalrts::Iter arg_end) {
  ...
  // </0 & CompileFile/4 s.GenMode#1/10 s.Opt#1/11 s.MarkupContext#1/12 (/15 e.SrcName#1/13 )/16 e.OutputName#1/2 '.'/9 'r'/8 'a'/7 's'/6 'l'/5 >/1
  context[0] = arg_begin;
  context[1] = arg_end;
  context[2] = 0;
  context[3] = 0;
  ...
  //RESULT: Tile{ [[ AsIs: </0 Reuse: & Fetch/4 } e.SrcName#1/13/17 </19 & Seq/20 </21 & FrontEnd/22 e.SrcName#1/13/23 >/25 </26 Tile{ HalfReuse: & @create_closure@/9 HalfReuse: & CompileFile\1/8 } Tile{ AsIs: s.MarkupContext#1/12 } Tile{ AsIs: s.Opt#1/11 } Tile{ AsIs: s.GenMode#1/10 } Tile{ AsIs: (/15 AsIs: e.SrcName#1/13 AsIs: )/16 } (/27 Tile{ AsIs: e.OutputName#1/2 } Tile{ HalfReuse: )/7 HalfReuse: >/6 HalfReuse: >/5 AsIs: >/1 ]] }
  if (! refalrts::copy_evar(context[17], context[18], context[13], context[14]))
    return refalrts::cNoMemory;
  if( ! refalrts::alloc_open_call( context[19] ) )
    return refalrts::cNoMemory;
  if( ! refalrts::alloc_name( context[20], ref_Seq.ref.function ) )
    return refalrts::cNoMemory;
  if( ! refalrts::alloc_open_call( context[21] ) )
    return refalrts::cNoMemory;
  if( ! refalrts::alloc_name( context[22], ref_FrontEnd.ref.function ) )
    return refalrts::cNoMemory;
  ...
  refalrts::reinit_name( context[8], ref_gen_CompileFile_L1.ref.function );
